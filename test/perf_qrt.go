...
var Tlapack, Tmatops, Rlapack, Rmatops *matrix.FloatMatrix
var ERRlapack, ERRmatops error
var decompOK, reflectorOK bool
...
func runCheck(A *matrix.FloatMatrix, LB int) (bool, time.Duration, time.Duration) {
    ...
    fnc := func() {
        _, ERRmatops = matops.DecomposeQRT(A, T, W, LB)
    }
    ...
    if verbose && N < 10 {
        ...
        fmt.Fprintf(os.Stderr, "T0:\n%v\n", T0)
    }
   // now A == A0 && tau == tau0

    decompOK = A.AllClose(A0)
    reflectorOK = T.AllClose(T0)

    if verbose {
        if ! decompOK {
            fmt.Fprintf(os.Stderr, "Decompose results not  equal\n")
        }
        if ! reflectorOK {
            fmt.Fprintf(os.Stderr, "Block reflector results not  equal\n")
        }
    }
    if ! decompOK || ! reflectorOK {
    ...
    return decompOK && reflectorOK, time0, time2
}
        ...
        if check {
            ...
            if ! ok {
                ...
                if ! decompOK {
                    ...
                    fmt.Fprintf(os.Stderr, "A col diffs:\n%v\n\n", cdf)

                    rdf := rowDiffs(Rmatops, Rlapack)
                    rdf.SetSize(1, rdf.Rows(), 1)
                    fmt.Fprintf(os.Stderr, "A row diffs:\n%v\n\n", rdf)
                }

                if ! reflectorOK {
                    ctf := columnDiffs(Tmatops, Tlapack)
                    ctf.SetSize(1, ctf.Rows(), 1)
                    fmt.Fprintf(os.Stderr, "T col diffs:\n%v\n\n", ctf)

                    rtf := rowDiffs(Tmatops, Tlapack)
                    rtf.SetSize(1, rtf.Rows(), 1)
                    if verbose {
                        fmt.Fprintf(os.Stderr, "T row diffs:\n%v\n\n", rtf)
                        fmt.Fprintf(os.Stderr, "T:\n%v\n", Tmatops)
                        fmt.Fprintf(os.Stderr, "Tlapack:\n%v\n", Tlapack)
                    }
                }
            }
            return
